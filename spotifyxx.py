...
def artistForGenres(genres):
    ...
    #  print(data2['tracks'])
    #  To show the unfiltered results

    print("The recommendation dictionary for the genres ", genres, "are: \n\n" )

    # This dictionary has the name of the artist as the key
    # and the spotify link to the artist's album and the artist uri as values
    for i in range (19):
        artistUri = (data2['tracks'][i]['album']['artists'][0]['uri'])
        name = (data2['tracks'][i]['album']['name'])
        link = (data2['tracks'][i]['album']['artists'][0]['external_urls'])
        genreInfo.setdefault(name, [])
        genreInfo[name].append(link)
        genreInfo[name].append(artistUri)

    p = 0
    for key, value in genreInfo.items():
        print(p, ". The name of the artist: ", key, ' :  The related info  ', value)
        p = p+1
